...
internal sealed class Game : MonoBehaviour
{
    ...
    private void Start()
    {
            ...
            private void Update()
        {
            ...
            foreach (Vector2Int position in SystemLocator.I.Game.PlayablePositions)
            {
                ...
                SystemLocator.I.Game.SetProductionProgressView(position, secondsPassed/order.DurationSeconds);
                if (secondsPassed < order.DurationSeconds || !CanAfford(order.Inputs)) continue;
               
                SystemLocator.I.Game.DisplayOrderDone(position, order);
        ...
        public bool CanAfford(params ResourceCount[] resourceCounts)
        {
            ...
            {
                if (SystemLocator.I.GameData.Resources[resourceCounts[i].Type] < resourceCounts[i].Count)
                {
                    return false;
                }
            }
        ...
        public void Spend(params ResourceCount[] resourceCounts)
        {
            for (int i = 0; i < resourceCounts.Length; i++)
            {
                SystemLocator.I.GameData.Resources[resourceCounts[i].Type] -= resourceCounts[i].Count;
            }
        }
    ...
    public CanUpgradeResult CanUpgrade(Cell cell)
    {
        ...
        if (!CanAfford(cost)) return CanUpgradeResult.InsufficientResources;
    ...
    public void DemolishBuilding(Cell cell)
    {
        ...
        BuildingLine buildingLine = SystemLocator.I.ContentLibrary.GetBuilding(cellData.Building.TypeId);
        Aquire(buildingLine.Levels[cellData.Building.Level].Cost);
